<template>
  ...
  >
    <div class="flex items-center mb-6">
      <button @click="goBack" class="mr-4 text-darkYellow hover:text-white transition">
        <svg
    ...
    <div v-if="teamMembers.length === 0" class="border border-white bg-black/70 p-6 text-white/80">
    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
      ...
      >
        ...
        <p v-if="member.jobTitle" class="text-darkYellow font-semibold mb-2 text-center">
          {{ member.jobTitle }}
        </p>
        <button
    ...
    >
      <div
        class="bg-black border border-white shadow-xl max-w-md w-full p-8 relative animate-fadeIn"
      >
        <button @click="closeModal" class="absolute top-3 right-3 text-darkYellow hover:text-black">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="h-6 w-6"
            fill="none"
        ...
        <div class="flex flex-col items-center">
          <img
            ...
            @error="onMemberImageError"
          />
          <h2 class="text-2xl font-bold text-darkYellow mb-2 text-center">
            {{ selectedMember.name || 'Unnamed Member' }}
          </h2>
          <p v-if="selectedMember.jobTitle" class="text-darkYellow font-semibold mb-2 text-center">
            {{ selectedMember.jobTitle }}
          </p>
          <p class="text-white text-center whitespace-pre-line">
            {{ selectedMember.about || 'More information coming soon.' }}
          </p>
        </div>
      </div>
    </div>
...
const teamMembers = computed(() => {
  ...
  const members = Array.isArray(block?.props?.members) ? block.props.members : []
