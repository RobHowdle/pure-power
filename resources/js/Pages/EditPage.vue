...
onMounted(async () => {
	...
	status.value = page.value.status;
...
const normalizeBlocks = (raw) => {
	...
	return raw
		.map((b) => {
			if (b && typeof b === "object" && "type" in b && "data" in b) {
				const id = b.id ?? createId();
				if (b.type === "text") {
					return {id, type: "text", props: {text: b.data ?? ""}};
				}
				if (b.type === "image") {
					return {
						id,
						type: "image",
						props: {src: b.data ?? "", alt: "", caption: ""},
					};
				}
			}
...
const getOrCreateBlock = (type, defaults = {}) => {
	let block = blocks.value.find((b) => b.type === type);
...
const ensureHomePrefill = () => {
	...
	const hasType = (type) => blocks.value.some((b) => b.type === type);
...
const ensureStandardPagePrefill = () => {
	...
	const legacyTextBlocks = blocks.value.filter((b) => b.type === "text");
...
const pageHeading = computed({
	get() {
		const block =
			blocks.value.find((b) => b.type === "page_heading") ??
			blocks.value.find((b) => b.type === "page_text");
	...
	set(value) {
		...
		// Remove legacy field once heading is explicitly updated.
		blocks.value = blocks.value.filter((b) => b.type !== "page_text");
	},
});
...
const removeTeamMember = (memberId) => {
	...
	block.props.members = block.props.members.filter((m) => m.id !== memberId);
};
...
const visibleBlocks = computed(() => {
	if (isHomePage.value) {
		...
		return order
			.map((type) => blocks.value.find((b) => b.type === type))
			.filter(Boolean);
	}
...
<template>
	<div v-if="page" class="py-12">
		<div class="mx-auto max-w-6xl sm:px-6 lg:px-8 flex gap-8">
			<aside
				...
				<button
					@click="updatePage"
					class="mt-4 w-full inline-flex justify-center items-center border border-darkYellow bg-darkYellow px-4 py-2 font-extrabold uppercase tracking-widest text-black hover:bg-lightYellow hover:border-lightYellow">
					Save Page
				</button>
			</aside>
